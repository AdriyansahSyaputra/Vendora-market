...
const AddProduct = ({ categories }) => {
  ...
  const handleImageChange = (e) => {
    ...
    if (files.length > 0) {
      ...
      const allPreviews = [...imagePreviews, ...newPreviews].slice(0, 5); // Limit to 5 images total
      setImagePreviews(allPreviews);
      form.setValue(
        "images",
        allPreviews.map((p) => p.file),
        { shouldValidate: true }
      );
    }
  };
  ...
  const toggleVariations = () => {
    setHasVariations((prev) => {
      ...
      if (newState && fields.length === 0) {
        // If switching to variations and there are none, add one.
        append({ size: "M", color: "", stock: 10 });
      } else if (!newState) {
        // If switching off variations, clear them.
  ...
  function onSubmit(data) {
    // In a real app, you would format the data and send it to your API
    console.log("Form Submitted:", data);
    alert("Product form submitted! Check the console for the data.");
  }
